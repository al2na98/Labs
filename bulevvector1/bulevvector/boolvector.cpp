...
void boolvector::Scan(int N) {
	delete[] bv; // Удаляем предыдущий массив, если он существует
	n = N;
	m = (n + 7) / 8; // Вычисляем количество байтов
	bv = new UC[m](); // Выделяем память для нового массива
...
boolvector boolvector::operator & (boolvector& x) {
	int t = min(n, x.n); // Длина результирующего вектора
	boolvector temp(t); // Создаем временный вектор
	for (int i = 0; i < temp.m; i++) {
		temp.bv[i] = (i < m ? bv[i] : 0) & (i < x.m ? x.bv[i] : 0);
	}
	return temp;
}
...
boolvector& boolvector::inversion(int K) {
	...
	int byteIndex = K / 8;
	int bitIndex = K % 8;
	bv[byteIndex] ^= (1 << (7 - bitIndex)); // Инвертируем бит
	return *this;
}
...
boolvector& boolvector::operator = (boolvector& x) {
	if (this != &x) { // Проверяем на самоприсваивание
		delete[] bv; // Удаляем старый массив
		n = x.n;
		m = x.m;
		bv = new UC[m]; // Выделяем память для нового вектора
		memcpy(bv, x.bv, m); // Копируем значения из вектора x
	}
...
boolvector& boolvector::setup1(int k) {
	return setup(k, 1); // Устанавливаем бит в 1
}
// Установка бита в 0 по индексу k
boolvector& boolvector::setup0(int k) {
	return setup(k, 0); // Устанавливаем бит в 0
}
// Установка нескольких битов в 1, начиная с индекса k
boolvector& boolvector::setup(int k, int p) {
	if (k < 0 || k >= n) {
		throw std::out_of_range("Индекс выходит за пределы");
	}
	int byteIndex = k / 8; // Находим индекс байта
	int bitIndex = k % 8; // Находим индекс бита

	if (p == 1) {
		bv[byteIndex] |= (1 << (7 - bitIndex)); // Устанавливаем бит в 1
	}
	else {
		bv[byteIndex] &= ~(1 << (7 - bitIndex)); // Устанавливаем бит в 0
	}
